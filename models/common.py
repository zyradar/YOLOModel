...
"""class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        # self.m = nn.AvgPool2d(2, 2)
        self.m = nn.AdaptiveAvgPool2d(output_size=(384, 640))

    def forward(self, x):
        # mark
        if x.shape[-1] == 3:
            return self.act(self.bn(self.conv(self.m(x.transpose(2, 3).transpose(1, 2)))))
        else:
            return self.act(self.bn(self.conv(x)))

    def fuseforward(self, x):
        if x.shape[-1] == 3:
            return self.act(self.conv(self.m(x.transpose(2, 3).transpose(1, 2))))
        else:
            return self.act(self.conv(x))"""
...
"""class Conv(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.shape[-1] == 3:
            chsample = CHsample()
            return self.act(self.bn(self.conv(chsample(x))))
        else:
            return self.act(self.bn(self.conv(x)))
    ...
    def fuseforward(self, x):
        if x.shape[-1] == 3:
            chsample = CHsample()
            return self.act(self.conv(chsample(x)))
        else:
            return self.act(self.conv(x))"""
