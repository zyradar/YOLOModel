...
def parse_model(d, ch, polygon=False):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
                 C3, C3TR, C3SPP, C3Ghost, C3x, nn.ConvTranspose2d]:
            c1, c2 = ch[f], args[0]
        ...
        #elif m in [BiFPN_Concat, BiFPN_Concat2, BiFPN_Concat3]:
        #    c2 = sum(ch[x] for x in f)
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                if self.inplace:
                    ...
                    """xy1, xy2, xy3, xy4, conf = x[i].split((2, 2, 2, 2, self.nc + 1), 4)
                    # xy1, xy2, xy3, xy4, conf = x[i].sigmoid().split((2, 2, 2, 2, self.nc+1), 4)
                    xy1 = (xy1 + self.grid[i]) * self.stride[i]
                    xy2 = (xy2 + self.grid[i]) * self.stride[i]
                    xy3 = (xy3 + self.grid[i]) * self.stride[i]
                    xy4 = (xy4 + self.grid[i]) * self.stride[i]
                    y = torch.cat((xy1, xy2, xy3, xy4, conf.sigmoid()), 4)"""
...
class Polygon_Model(Model):
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Polygon_Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            b = mi.bias.view(m.na, -1).detach()  # conv.bias(267) to (3,89)
