...
from models.common import Conv, Bottleneck, SPP,SPPF, DWConv, Focus, BottleneckCSP, C3, ShuffleV2Block, Concat, NMS, autoShape, StemBlock, BlazeBlock, DoubleBlazeBlock
    ...
    def forward(self, x):
        ...
        if self.export_cat:
            for i in range(self.nl):
                ...
                # landm1 = x[i][:, :, :, :, 5:7] * self.anchor_grid[i] + self.grid[i].to(x[i].device) * self.stride[i]  # landmark x1 y1
                # landm2 = x[i][:, :, :, :, 7:9] * self.anchor_grid[i] + self.grid[i].to(x[i].device) * self.stride[i]  # landmark x2 y2
                # landm3 = x[i][:, :, :, :, 9:11] * self.anchor_grid[i] + self.grid[i].to(x[i].device) * self.stride[i]  # landmark x3 y3
                # landm4 = x[i][:, :, :, :, 11:13] * self.anchor_grid[i] + self.grid[i].to(x[i].device) * self.stride[i]  # landmark x4 y4
                # landm5 = x[i][:, :, :, :, 13:15] * self.anchor_grid[i] + self.grid[i].to(x[i].device) * self.stride[i]  # landmark x5 y5
                x[i][..., 5:15]= x[i][..., 5:15] * self.anchor_grid[i].repeat((1, 1, 1, 1, 5))+self.grid[i].to(x[i].device).repeat((1, 1, 1, 1, 5)) * self.stride[i]
                y = torch.cat((x[i][..., 4:5].sigmoid(),x[i][..., 5:15]), -1)
                # y = torch.cat((x[i][:, :, :, :, 4:5].sigmoid(),x[i][:, :, :, :, 5:15]), 4)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, Bottleneck, SPP,SPPF, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, C3, ShuffleV2Block, StemBlock, BlazeBlock, DoubleBlazeBlock]:
            c1, c2 = ch[f], args[0]
