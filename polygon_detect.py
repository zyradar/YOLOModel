            ...
            if len(det):
                ...
                for *xyxyxyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        xyxyxyxyn = [xyxyxyxyn[0], xyxyxyxyn[1], xyxyxyxyn[6], xyxyxyxyn[7], xyxyxyxyn[4], xyxyxyxyn[5], xyxyxyxyn[2], xyxyxyxyn[3]]
                        # print("xyxyxyxyn", xyxyxyxyn)
                        # print(1222)
                        cv2.putText(im0, str(cls),
                                    (int((xyxyxyxyn[0]*1920)) - 50, int(xyxyxyxyn[1]*1200) + 10),
                                    cv2.FONT_HERSHEY_DUPLEX, 3, (0, 255, 0), 1)
                        line = (cls, *xyxyxyxyn, conf) if save_conf else (cls, *xyxyxyxyn)  # label format

                        cv2.waitKey(0)  # 1 millisecond
                        with open(txt_path + '.txt', 'a') as f:
                            f.write(('%g ' * len(line)).rstrip() % line + '\n')
                    # label = (torch.tensor(cls).view(0)).view(-1).tolist()
                    # print(int(cls))
                    xyxyxyxyn = (torch.tensor(xyxyxyxy).view(1, 8)).view(-1).tolist()  # normalized xyxyxyxy
                    xyxyxyxyn = [xyxyxyxyn[0], xyxyxyxyn[1], xyxyxyxyn[6], xyxyxyxyn[7], xyxyxyxyn[4], xyxyxyxyn[5], xyxyxyxyn[2], xyxyxyxyn[3]]
                    lable = names[int(cls)]
                    print(conf)
                    cv2.putText(im0, str(lable),
                                (int(xyxyxyxyn[0]) - 50, int(xyxyxyxyn[1]) + 10),
                                cv2.FONT_HERSHEY_DUPLEX, 3, (0, 255, 0), 1)
                    # print(xyxyxyxyn)
                    # cv2.rectangle(im0, c1, c2, (0, 255, 0), thickness=1, lineType=cv2.LINE_AA)
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        print(label)
                        # print(im0.shape[0], im0.shape[1])
                        # cv2.putText(im0, label, c1, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), thickness=2, lineType=cv2.LINE_AA)
                """for *xyxyxyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        xyxyxyxyn = (torch.tensor(xyxyxyxy).view(1, 8) / gn).view(-1).tolist()  # normalized xyxyxyxy
                        line = (cls, *xyxyxyxyn, conf) if save_conf else (cls, *xyxyxyxyn)  # label format
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    if vid_path != save_path:  # new video
                        ...
                        # cv2.putText(im0, label, (int(xyxyxyxyn[0]) - 50, int(xyxyxyxyn[1]) + 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), thickness=2,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='D:/mathroi/polygon_last_575.pt', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='C:/Users/HZY/MVviewer/videos/A5201CU150_AM00990AAK00003/Video_2023_03_17_201344_5.avi', help='file/dir/URL/glob, 0 for webcam')
