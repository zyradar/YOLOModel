...
from models.common import BiFPN_Add2, BiFPN_Add3, BiFPN_Concat
...
def train(hyp, opt, device, tb_writer=None, polygon=False):
    ...
    for k, v in model.named_modules():
        ...
        # BiFPN_Concat
        # elif isinstance(v, BiFPN_Concat) and hasattr(v, 'w') and isinstance(v.w, nn.Parameter):
        #    pg0.append(v.w)
        ...
        if rank in [-1, 0]:
            ...
            if epoch % 25 == 0:
                ckpt = {'epoch': epoch,
                        ...
                        'optimizer': optimizer.state_dict(),
                        'wandb_id': wandb_logger.wandb_run.id if wandb_logger.wandb else None}
                pt_name = 'polygon_last_' + str(epoch) + '.pt'
                la = wdir / pt_name
                torch.save(ckpt, la)
                strip_optimizer(la)  # strip optimizers
                del ckpt
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='models/yolov5s.pt', help='initial weights path')     # /home/evolution/文档/server/polygondetect/runs/train/exp92/weights/polygon_last.pt
    parser.add_argument('--cfg', type=str, default='models/yolov5s_1280.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='data/leida.yaml', help='dataset.yaml path')
